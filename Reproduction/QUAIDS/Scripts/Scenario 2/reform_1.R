...
# 1) Viés por bem no holdout
with(mm$by_good, setNames(rmse - mae*0, share))  # (apenas atalho p/ ver magnitudes)
colMeans(dat_test[, share_cols] - preds_test)     # viés médio por bem

# 2) Erro por quantil de despesa (heterocedasticidade)
qt <- cut(dat_test$lndespesa, quantile(dat_test$lndespesa, 0:5/5, na.rm=TRUE), include.lowest=TRUE)
e  <- rowSums((as.matrix(dat_test[, share_cols]) - preds_test)^2)^(1/2)
tapply(e, qt, mean, na.rm=TRUE)

# 3) Painel de dispersão y_true vs y_pred no holdout (por bem)
par(mfrow=c(2,3)); for (j in seq_along(share_cols)) {
  ys <- dat_test[[share_cols[j]]]; xp <- preds_test[, j]
  plot(xp, ys, pch=20, cex=.6, xlab="pred", ylab="obs", main=share_cols[j]); abline(0,1,col=2)
}
par(mfrow=c(1,1))


# =========================
# Calibração pós-hoc p/ shares (OOF no treino)
# =========================
project_rows_simplex <- function(M) {
  M <- as.matrix(M); M[M < 0] <- 0
  rs <- rowSums(M); zero <- !is.finite(rs) | rs <= 0
  if (any(zero)) M[zero, ] <- 1 / ncol(M)
  sweep(M, 1, rowSums(M), "/")
}

get_oof_preds_Ls <- function(dat, K=5, seed=123, tau=tau_holdout) {
  set.seed(seed)
  n  <- nrow(dat); k <- length(shareNames)
  oof <- matrix(NA_real_, n, k, dimnames=list(NULL, shareNames))
  ok  <- complete.cases(dat[, c(shareNames, lnpriceNames, "z"), drop=FALSE])
  ids <- which(ok); folds <- sample(rep_len(1:K, length.out=length(ids)))
  for (f in 1:K) {
    test_id  <- ids[folds == f]
    ...
    LN <- as.matrix(dat[test_id, lnpriceNames, drop=FALSE])
    P  <- exp(LN)
    # índice de Stone com wbar do fold
    Wbar <- colMeans(dat[train_id, shareNames, drop=FALSE], na.rm=TRUE)
    Wbar <- pmax(Wbar, 1e-12); Wbar <- Wbar / sum(Wbar)
    base_lnP <- rowSums(LN * matrix(as.numeric(Wbar), nrow(LN), length(Wbar), byrow=TRUE))
    X <- exp(dat$z[test_id] + base_lnP)
    for (i in seq_along(test_id)) {
      wi <- try(predict_shares_generic(fit_k, x=X[i], p=P[i,], tau=tau,
                                       normalize="softmax", eps=1e-6), silent=TRUE)
      if (!inherits(wi, "try-error") && all(is.finite(wi))) oof[test_id[i], ] <- wi
    }
  }
  oof
}
...
# 1) OOF no train
oof_train <- get_oof_preds_Ls(dat_train, K=5, seed=123, tau=tau_holdout)
y_train   <- as.matrix(dat_train[, shareNames, drop=FALSE])

# 2) Calibrador (a) shift por bem (remove viés médio no train-OOF)
cal_shift <- colMeans(y_train - oof_train, na.rm=TRUE)
apply_shift <- function(P) {
  M <- sweep(as.matrix(P), 2, cal_shift, "+")
  project_rows_simplex(M)
}

# 3) Calibrador (b) afim por bem (y ≈ a + b*pred), com projeção no simplex
fit_affine <- function(y, x) {
  fits <- lapply(seq_along(shareNames), function(j) {
    ok <- is.finite(y[, j]) & is.finite(x[, j])
    if (!any(ok)) return(c(a=0, b=1))
    cf <- coef(lm(y[ok, j] ~ x[ok, j]))
    c(a=unname(cf[1]), b=unname(cf[2]))
  })
  do.call(rbind, fits) |> `rownames<-`(shareNames)
}
cal_aff <- fit_affine(y_train, oof_train)
...
# 4) Aplica no holdout
preds_shift  <- apply_shift(preds_test)
preds_affine <- apply_affine(preds_test)

# 5) Recalcula métricas
metr <- function(pred) {
  yt <- dat_test[, shareNames, drop=FALSE]
  rmse <- sqrt(mean((as.matrix(pred) - as.matrix(yt))^2, na.rm=TRUE))
  mae  <- mean(abs(as.matrix(pred) - as.matrix(yt)), na.rm=TRUE)
  c(rmse=rmse, mae=mae,
    bias2 = sum(colMeans(yt - pred, na.rm=TRUE)^2))  # soma de viés^2 por bem (diagnóstico)
}

base_m <- metr(preds_test)
sh_m   <- metr(preds_shift)
af_m   <- metr(preds_affine)

cat("\n== Holdout (antes / shift / afim) ==\n")
print(rbind(base=base_m, shift=sh_m, affine=af_m), digits=6)

# 6) Bias por bem (antes vs calibrado)
bias_tbl <- cbind(
  base   = colMeans(dat_test[, shareNames, drop=FALSE] - preds_test,    na.rm=TRUE),
  shift  = colMeans(dat_test[, shareNames, drop=FALSE] - preds_shift,   na.rm=TRUE),
  affine = colMeans(dat_test[, shareNames, drop=FALSE] - preds_affine,  na.rm=TRUE)
)
print(round(bias_tbl, 6))
